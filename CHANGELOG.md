...
- **In Audio, the subtitles burned into a transcribed video can be styled and previewed before exporting.** Typeface, size, colour, box or outline, position and margins, shown over a real frame of your own clip with its longest caption.
- **A negative prompt in the image and video generators.** Video comes prefilled with the model's recommended one and can be replaced or emptied; in images it only takes effect above CFG 1, and the app says so on the models that ignore it.
...
- **In Audio, the subtitle size setting works across its whole range.** Below the middle of the slider every value rendered the same, because an internal floor was measured in points against a preview still rather than the video.
- **In Audio, a captioned video keeps the codec and the colour of the original.** An HEVC clip was re-encoded as H.264, and a wide-gamut or HDR one came back flattened.
- **In Audio, the transcript keeps the view you chose once a translation finishes.** It dropped to translation-only even with bilingual selected, and switching away and back was the only way to get it back.
- **In Audio, long subtitles are no longer cut off in the exported video.** The caption box was a fixed height, so anything that needed more lines than fitted was clipped. It now grows to the text, and shrinks the text only when it would not fit on screen.
