...
### Improved
- **Models that answer to several people at once write up to 40% faster.** Serving two or three conversations from the same engine made every one of them re-read the whole model for each reply, because the batched path was only used from four conversations up. A 9B at Q5_K_M goes from 55.8 to 71.9 tokens/s with two, and from 59.9 to 84.3 with three. Four or more is unchanged.
- **The smallest quantisations write answers up to 54% faster.** Their unpacking table was read from the slowest memory the card has, one byte at a time; it is now kept next to the compute units and read in full words. A 1.5B at IQ2_M goes from 94.6 to 145.4 tokens/s. Output is identical, down to the fourth decimal of perplexity.
...
- **The experimental turbo key-value cache is now the fastest option, not the slowest.** Its rotation step picked one of two sign tables inside the loop, so every element addressed both, and it moved the data one number at a time. A 7B at 4096 tokens of context goes from 55.9 to 59.2 tokens/s with turbo3 and from 56.9 to 61.1 with turbo4, which puts both ahead of q8_0 at 58.0. Perplexity on a 9B is unchanged at 7.7694.
