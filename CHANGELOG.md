...
- **Speculative decoding carries the gain too.** On a Qwen3.5 4B writing code, MTP goes from 63.6 to 72.6 tokens/s and DFlash from 68.9 to 82.6; on mixed unpredictable text DFlash goes from 60.7 to 68.3. The draft acceptance rate is identical down to the last counter in every pair, so the speedup is the kernels and not a different acceptance pattern.
