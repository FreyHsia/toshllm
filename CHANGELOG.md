...
- **Q4_K models serve four or more conversations 12% faster, and gain 8% with multi-token prediction.** Every row group of a threadgroup was reading the same activation values from memory, and adding them up again for the correction term of each block; both now happen once, in shared memory. On a Radeon RX Vega 64, a dense 4B at Q4_K_M goes from 108.3 to 121.1 tokens/s with four conversations and from 119.5 to 133.6 with eight, and a Qwen3.5 4B with multi-token prediction from 65.5 to 70.5 on predictable text, writing the same answer with the same acceptance rate. One or two conversations are unchanged, since the shared copy only pays from three columns up. `TOSH_EXT_LDS_DISABLE=1` restores the old path.
