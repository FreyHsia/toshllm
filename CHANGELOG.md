...
- **Serving several conversations at once is 9% faster, and speculative decoding pays off again.** The batched matrix-vector step split its work for narrower compute units. On a 9B at Q5_K_M with four conversations it goes from 47.4 to 51.6 tokens/s. Multi-token prediction now reaches 58.8 tokens/s against 54.0 without it on predictable text, and it steps aside earlier when the draft stops being accepted, which halves the cost it used to add there. `TOSH_EXT_NXPSG_X2_DISABLE=1` and `TOSH_MTP_ACC_MIN` restore the old behaviour.
