...
- **The smallest quantisations write answers about 49% faster.** Their unpacking table was read from the slowest memory the card has, one byte at a time; it is now kept next to the compute units and read in full words. A 1.5B at IQ2_M goes from 93.7 to 139.9 tokens/s, and a 7B that uses the same type for part of its weights from 50.4 to 58.3. Output is identical, down to the fourth decimal of perplexity.
