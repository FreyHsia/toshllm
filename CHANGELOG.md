...
- **The installer window is a designed one.** Opening the DMG now shows the app and the Applications folder on their own cards over the same gradient as the icon, with an arrow between them, and the disk carries the app's icon. The layout ships as a saved window arrangement, so the release built by CI looks the same as one built by hand.
