...
- **DeepSeek and GLM models read long prompts up to 31% faster.** Their attention keeps keys wider than values, a shape the fast prompt path could not express, so they fell back to a slower one. A 23B GLM goes from 203.8 to 257.5 tokens/s on a 512-token prompt with 4096 already in context, and from 121.5 to 159.2 with 8192. The deeper the conversation, the larger the gain. Start the engine with `TOSH_FA_PF_MLA_DISABLE=1` to go back to the old path.
- **Models with q5_0 or q5_1 weights read prompts about 15% faster.** Their fifth bit was assembled into an integer with a shift, a mask and an or for every single weight; it is now a conditional add of a value computed once per block, the way the K-quants already did it. A 9B vision model with 13% of its weights in q5_0 goes from 739.4 to 758.2 tokens/s on a 512-token prompt.
- **The experimental turbo key-value cache stops being the slowest option.** Its rotation step picked one of two sign tables inside the loop, so every element addressed both, and it moved the data one number at a time. On a 7B with no context yet, turbo3 goes from 66.6 to 73.0 tokens/s and turbo4 from 66.9 to 73.2, against 70.6 for q8_0 on the same run; at 4096 tokens of context the two go from 55.8 to 59.8 and from 56.8 to 60.5, against 58.1. Both were behind q8_0 before and lead it now. Perplexity on a 9B is unchanged at 7.7694.
- **Prompt reading improves on models quantised with the IQ family.** The wider matrix tile that other types already used now also serves four of them; a dense 7B at IQ3_XXS goes from 801.6 to 859.7 tokens/s on a 512-token prompt.
...
- **Cards with 64-wide compute units read long prompts 3 to 4% faster.** The wider matrix tile was switched off for them a while ago because it measured slower; it has since been improved and now wins, but only from about 450 tokens onwards, so that is where it starts. Below that they keep the path they had, unchanged. On a Radeon RX Vega 64, llama-2-7B goes from 511 to 529 tokens/s at 448 and from 509 to 528 at 512, and perplexity is identical. Start the engine with `TOSH_MM_WIDE_W64_DISABLE=1` to go back. Cards with 32-wide compute units are not affected.
